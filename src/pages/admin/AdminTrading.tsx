  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="font-display text-3xl font-bold">{t("admin.trading_control")}</h1>
          <p className="text-muted-foreground">{t("admin.trading_desc", "Monitor market activity and system controls.")}</p>
        </div>
        ...
        >
          {state.settings.tradingEnabled ? (
            <><Pause className="h-5 w-5" /> {t("admin.emergency_pause", "EMERGENCY PAUSE TRADING")}</>
          ) : (
            <><Play className="h-5 w-5" /> {t("common.resume", "RESUME TRADING")}</>
          )}
        </button>
      </div>
      ...
      {!state.settings.tradingEnabled && (
        <div className="bg-destructive/20 border border-destructive/50 text-destructive rounded-2xl p-6 flex items-start gap-4">
          ...
          <div>
            <h3 className="font-bold text-lg">{t("admin.trading_halted_title", "TRADING IS CURRENTLY HALTED")}</h3>
            <p className="opacity-90">{t("admin.trading_halted_desc", "All new orders are being rejected. Open positions cannot be closed until trading is resumed.")}</p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <div className="glass rounded-3xl p-6">
            <h3 className="font-display font-semibold mb-6 flex items-center gap-2">
              <Activity className="h-5 w-5 text-primary" /> {t("admin.live_order_flow", "Live Order Flow Volume")}
            </h3>
            <div className="h-64">
          ...
          <div className="glass rounded-3xl p-6">
            <h3 className="font-display font-semibold mb-4">{t("admin.system_parameters", "System Parameters")}</h3>
            <div className="space-y-4">
              <div className="flex items-center justify-between p-4 rounded-xl border border-border/50 bg-muted/20">
                <div>
                  <div className="font-semibold text-sm">{t("admin.max_leverage", "Max Leverage Allowed")}</div>
                  <div className="text-xs text-muted-foreground">{t("admin.max_leverage_desc", "Highest leverage multiplier for users")}</div>
                </div>
                <div className="font-mono bg-muted px-3 py-1 rounded-md">100x</div>
              </div>
              <div className="flex items-center justify-between p-4 rounded-xl border border-border/50 bg-muted/20">
                <div>
                  <div className="font-semibold text-sm">{t("admin.margin_threshold", "Margin Call Threshold")}</div>
                  <div className="text-xs text-muted-foreground">{t("admin.margin_threshold_desc", "Liquidation warning percentage")}</div>
                </div>
              ...
              <button className="flex items-center justify-center gap-2 w-full py-3 rounded-xl border border-border/50 hover:bg-muted transition-colors text-sm font-semibold text-muted-foreground">
                <Settings2 className="h-4 w-4" /> {t("common.update")} {t("admin.parameters", "Parameters")}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="glass rounded-3xl p-6">
            <h3 className="font-display font-semibold mb-4">{t("admin.top_traded", "Top Traded Assets")}</h3>
